              ...
              >
                <div className={`flex items-start gap-6 pl-20 lg:pl-0 ${i % 2 === 0 ? 'lg:justify-end' : 'lg:justify-start lg:[direction:ltr]'}`}>
                  ...
                  <div className={`max-w-lg ${i % 2 === 0 ? 'lg:text-right' : 'lg:text-left'}`}>
                    ...
                    <h3 className="text-2xl font-bold text-navy-900 dark:text-white mb-3">{t(`process.steps.${i}.title`, step.title)}</h3>
                    <p className="text-navy-600 dark:text-navy-300 leading-relaxed mb-4">{t(`process.steps.${i}.description`, step.description)}</p>
                  </div>
                </div>
                  ...
                  <ul className="space-y-3">
                    {step.details.map((d, j) => (
                      <li key={j} className="flex items-start gap-3">
                        ...
                        <span className="text-navy-700 dark:text-navy-300">{t(`process.steps.${i}.includes.${j}`, d)}</span>
                      </li>
                    ))}
                  </ul>
                </div>
